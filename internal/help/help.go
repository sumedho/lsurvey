...
func All() []Command {
	return []Command{
		...
		{Group: "Project & Session", Name: "recover", Usage: "recover <file>", Description: "Load the previous valid save as an unsaved project; preserves the original file.", Examples: []string{"recover job"}, Notes: []string{"The current file store keeps job.srv.bak after replacing a valid save. Recovery clears session undo/redo. Use saveas with a new path. Unsaved edits are not backed up."}},
		{Group: "Project & Session", Name: "check", Usage: "check", Description: "Check project integrity and report CRS and contour warnings without changing the project.", Examples: []string{"check"}, Notes: []string{"Integrity checks do not certify survey accuracy or replace observation QA."}},
		{Group: "Project & Session", Name: "save", Usage: "save [file]", Description: "Save the current project in a SQLite transaction. The .srv extension is appended when missing.", Examples: []string{"save", "save job"}, Notes: []string{"Legacy JSON files are read-only; use saveas with a new path to convert to SQLite. Existing saves retain a previous-state .srv.bak snapshot."}},
		...
		{Group: "Traverse", Name: "trav report", Usage: "trav report [compass|transit]", Description: "Preview horizontal closure, relative precision, course corrections and adjusted coordinates without editing. Defaults to compass.", Examples: []string{"trav report", "trav report transit"}},
		{Group: "Traverse", Name: "trav adjust", Usage: "trav adjust compass|transit", Description: "Apply a horizontal compass (length-weighted) or transit (axis-component-weighted) adjustment after closing.", Examples: []string{"trav adjust compass"}, Notes: []string{"Elevations remain unchanged. No angular adjustment or uncertainty estimate is performed. The QA report is retained in audit history. Start a new traverse after adjustment, or undo to revise it."}},
		...
		{Group: "Interface", Name: "results", Usage: "results", Description: "Open retained command output (F6).", Examples: []string{"results"}, Notes: []string{"Arrows/page keys and mouse wheel scroll; [ and ] select previous/next results.", "c copies output; e exports to a new text file without overwriting. The last 100 results are retained only for this TUI session.", "Esc returns to the originating screen."}},
