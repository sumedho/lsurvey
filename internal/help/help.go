...
func All() []Command {
	return []Command{
		...
		{Group: "Points", Name: "pt add", Usage: "pt add <id> <north> <east> [elev] [code]", Description: "Create a coded 2D or 3D point.", Examples: []string{"pt add 1 1000 2000 52.4 PEG", "pt add 2 1010 2010 TREE"}},
		{Group: "Points", Name: "pt edit", Usage: "pt edit <id> [north=] [east=] [elev=] [code=] [desc=]", Description: "Edit point fields.", Examples: []string{"pt edit 1 code=PEG desc=corner"}},
		...
		{Group: "Calculations", Name: "radiate", Usage: "radiate <from> <azimuth|bearing> <distance> [vdiff <delta>] as <id> [code]", Description: "Create a point from a start point, direction, and distance.", Examples: []string{"radiate 1 90.0000 10 as 2 CALC", "radiate 1 N 45.0000 E 50 vdiff 1.2 as 3 PEG"}},
		...
		{Group: "Contours", Name: "contour gen", Usage: "contour gen <id> <interval> [base=<elev>] [index=<n>] [breaklines=all|none|ids:L1,L2]", Description: "Generate and store contour polylines from elevated points and selected breaklines.", Examples: []string{"contour gen C1 1", "contour gen C1 0.5 base=100 index=5 breaklines=all"}},
		{Group: "Contours", Name: "contour list", Usage: "contour list", Description: "Report the number of stored contour sets.", Examples: []string{"contour list"}},
		{Group: "Contours", Name: "contour info", Usage: "contour info <id>", Description: "Show interval, base, polyline count, and breakline count for a contour set.", Examples: []string{"contour info C1"}},
		...
		{Group: "Lines and Export", Name: "line add", Usage: "line add <id> <p1> <p2> [code]", Description: "Create a coded line between two points.", Examples: []string{"line add L1 1 2 BOUNDARY"}},
		{Group: "Lines and Export", Name: "line edit", Usage: "line edit <id> [from=] [to=] [code=] [desc=]", Description: "Edit line endpoints, code, or description. Quote descriptions containing spaces.", Examples: []string{`line edit L1 code=EASE desc="access easement"`}},
		...
		{Group: "Import and Export", Name: "import csv", Usage: "import csv <file>", Description: "Import points from CSV with header id,northing,easting,elevation,code,description.", Examples: []string{"import csv points"}},
		{Group: "Import and Export", Name: "export csv", Usage: "export csv <file>", Description: "Export sorted points to CSV with header id,northing,easting,elevation,code,description.", Examples: []string{"export csv points"}},
		...
		{Group: "TUI", Name: "map", Usage: "map [lines|fit|zoom in|zoom out]", Description: "Toggle the full-screen ASCII point map, line overlay, zoom, or fit-to-points view.", Examples: []string{"map", "map lines", "map zoom in", "map fit"}},
		{Group: "TUI", Name: "help", Usage: "help [command]", Description: "Open full-screen command help.", Examples: []string{"help", "help radiate"}},
...
func Render(query string) string {
	...
	b.WriteString("\nKeys: F1 opens help, F2 toggles map, / starts a filter command, Tab accepts a command completion, Ctrl+N/Ctrl+P cycle completions, Up/Down browse command history, arrow keys pan map, +/- zoom map, f fits map, l toggles map lines, alt+s cycles sort fields, alt+d toggles direction, Esc closes help/map, Ctrl+C quits.\n")
	return b.String()
}
...
func RenderStyled(query string) string {
	...
	b.WriteByte('\n')
	b.WriteString(helpKeyStyle.Render("Keys: F1 help  Esc close  Tab complete  Ctrl+N/Ctrl+P completions  Up/Down history  F2 map  arrows pan map  +/- zoom  f fit  l lines"))
	return b.String()
}
