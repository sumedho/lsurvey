...
func TestRenderIncludesAngleRuleAndCommands(t *testing.T) {
	got := Render("")
	for _, want := range []string{"dd.mmsshhhh", "pt add", "rad", "close <p1> <p2> <p3> ...", "bearing add <a> <b>", "dist sub <a> <b>", "shift <base>", "rotate <base>", "scale apply <base>", "transform fit <src1>", "line gen <code>", "export dxf", "export geojson", "filter <text>", "desc <project description>", "i toggles map point ID/code labels"} {
...
func TestSuggestionsIncludeUsageAndExamples(t *testing.T) {
	...
	for _, want := range []string{"rad <from>", "save job", "trav show", "close 1 2 3", "bearing add <a> <b>", "dist sub 12.5 15", "trav leg <azimuth|bearing> <distance> [vdiff <delta>] [code]", "import geojson <file>", "export geojson <file>", "shift <base> [east=<coordinate>] [north=<coordinate>] [elev=<coordinate>]", "rotate 100 -15.3000", "scale apply <base> csf=<factor> [system=<label>]", "scale reverse", "transform fit <src1> <dst1>", "line gen <code>"} {
		if !strings.Contains(got, want) {
